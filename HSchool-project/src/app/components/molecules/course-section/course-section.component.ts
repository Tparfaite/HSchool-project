...
export class CourseSectionComponent implements OnInit{
  ...
  addCourse(course: Course): void {
    if (!this.coursesComponent.editMode && !this.coursesComponent.updateMode) {
      ...
      this.selectedCourse.progress = course.progress;
      this.selectedCourse.courseNo = course.courseNo;
  ...
  updateProgress(course: Course){
    ...
    const courseNoInput = document.querySelector('[formControlName="courseNo"]');
     if (courseNoInput) {
     (courseNoInput as HTMLInputElement).readOnly = true;
     courseNoInput.classList.add("text-gray-500")
    }
